...
| `create` | Multiple `-f/--file` inputs merge dependencies in order | `mamba/micromamba/tests/test_create.py#test_create_with_multiple_files` | `crates/viper-cli/tests/cli_smoke.rs` `create_merges_multiple_env_files_specs` |
| `create` | Conflicting YAML names across files fail | `mamba/micromamba/tests/test_create.py` multi-file target-name checks | `crates/viper-cli/tests/cli_smoke.rs` `create_rejects_conflicting_names_across_env_files` |

...
| Repodata | Relaxed specs use `current_repodata.json` in offline mode | `mamba/libmamba/src/solver/helpers.cpp` + `mamba/libmamba/src/core/subdir_index.cpp` current-repodata-first strategy | `crates/viper-cli/tests/cli_smoke.rs` `offline_relaxed_spec_uses_current_repodata_without_full_cache` |
| Repodata | Restrictive specs require `repodata.json`; missing full cache fails in offline mode | `mamba/libmamba/src/solver/helpers.cpp` full-repodata trigger semantics + `mamba/libmamba/src/core/subdir_index.cpp` offline miss behavior | `crates/viper-cli/tests/cli_smoke.rs` `offline_restrictive_spec_requires_full_repodata_cache` |
| Repodata | Restrictive specs consume `repodata.json` when available and can resolve versions absent from `current_repodata.json` | `mamba/libmamba/src/solver/helpers.cpp` full-repodata trigger semantics | `crates/viper-cli/tests/cli_smoke.rs` `offline_restrictive_spec_uses_full_repodata_when_available` |

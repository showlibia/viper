...
| `install` | `MAMBA_TARGET_PREFIX` has precedence over `CONDA_PREFIX` when no CLI target is provided | `mamba/micromamba/tests/test_install.py` target-prefix environment precedence matrix | `crates/viper-cli/tests/cli_smoke.rs` `install_print_config_only_prefers_mamba_target_prefix_over_conda_prefix`, `install_prefers_mamba_target_prefix_over_conda_prefix_non_print_path` |
...
| `remove` | Empty-root prune case still removes orphan dependencies while avoiding empty-target solver regression | `mamba/libmamba/src/api/remove.cpp` `Keep + Remove(clean_dependencies=true)` semantics | `crates/viper-cli/tests/cli_smoke.rs` `remove_default_prune_removes_orphans_when_no_requested_roots_remain`, `remove_with_unparseable_history_keeps_unrelated_conda_packages` |

...
| `create` | Repeated non-YAML spec files merge in order (classic + explicit), mixed file types fail explicitly | `mamba/micromamba/tests/test_create.py#test_create_with_multiple_files`; `mamba/libmamba/src/api/install.cpp` `file_specs_hook` type consistency | `crates/viper-cli/tests/cli_smoke.rs` `create_merges_multiple_classic_spec_files`, `create_merges_multiple_explicit_files`, `create_rejects_mixed_file_spec_types` |
